...
| **Guard** | Is the policy failing *right now*? Mahalanobis plus chunk-consistency scores with time-uniform conformal thresholds calibrated on your own successful rollouts. Emits ok / slow / handover / stop. Measured: 20/20 detection of miscalibration and erratic-action faults at 0.04 s median latency with 0/17 false alarms; blind to stalls, see [Known limits](#known-limits). | `robotruth guard` |
...
- **Guard detection, measured with faults that actually break the policy** (30/30 injected faults failed the task): offset and noise faults detected 20/20 at **0.04 s median latency** with **0/17 false alarms** on a 164-episode calibration pool; stall faults missed 0/10, a documented scorer blind spot. Small 45-episode pools leaked up to 3/17 false alarms, so the calibration-size guidance is measured, not guessed.
...
- **The guard's measured operating envelope** (164-episode calibration pool, live policy, faults injected into executed actions): it detects miscalibration and erratic-policy faults in 10/10 and 10/10 episodes with a median latency of 0.04 s and 0/17 false alarms on held-out successes. It is blind to stall faults (0/10): a frozen action stream stays self-consistent and inside the nominal feature manifold, so distance and consistency scorers cannot see it. A stagnation scorer is the planned fix. Calibrate on 150 or more nominal episodes; 45-episode pools leaked up to 3/17 false alarms in the same experiment. Whatever the benchmark says, measure the envelope again on your own robot with `robotruth guard metrics`; the numbers above are one policy on one task.
