...
| **Guard** | Is the policy failing *right now*? Mahalanobis, chunk-consistency and action-statistics scores in one head, a stagnation score in its own head, both with time-uniform conformal thresholds calibrated on your own successful rollouts. Emits ok / slow / handover / stop. Measured: **30/30 detection** of stall, miscalibration and erratic-action faults at **0.04 s median latency** with **0/18 false alarms**. | `robotruth guard` |
...
- **The guard's measured operating envelope** (150-episode calibration pool, live policy, faults injected into executed actions): stall, miscalibration and erratic-policy faults are each detected 10/10, median latency 0.04 s, with 0/18 false alarms on held-out successes. Three caveats stand. The faults are injected into the executed action, which is a clean, abrupt change; a slow degradation is a harder problem and is not measured here. The `bonferroni` method still leaked up to 4/18 false alarms at 45 calibration episodes, so `max` remains the default and 150 or more nominal episodes remains the guidance. The stall head needs roughly 40 calibration episodes to certify its share of alpha; below that it reports itself as saturated and stall detection is disabled rather than silently unguaranteed. Measure the envelope again on your own robot with `robotruth guard metrics`; the numbers above are one policy on one task.
